...
function Ensure-File(
  ...
  if ((Test-Path $Path) -and -not $Force) { Write-Step "Exists: $Path (skip; use -Force to overwrite)"; return }
...
function Ensure-GitIdentity() {
  ...
  $nameInvalid = [string]::IsNullOrWhiteSpace($name) -or ($name -match '^[\\]+$')
  $emailInvalid = [string]::IsNullOrWhiteSpace($email) -or ($email -notmatch '@')

  if ($nameInvalid) { Git-Run 'config user.name AutoUser' }
  if ($emailInvalid) { Git-Run 'config user.email auto@example.com' }
}
